...
GREENFIELD_PIPELINE: tuple[PipelinePhase, ...] = (
    PipelinePhase(
        ...
        (
            ...
            "first_revenue_target_date recorded",
            "next_revenue_test defined",
            "test_owner assigned",
            "expected_cost recorded",
            "success_threshold defined",
        ),
    ),
    PipelinePhase(
        "revenue_test",
        "Pre-build demand and willingness-to-pay test",
        AgentRole.GROWTH,
        (
            "cheapest credible demand test initiated",
            "commercial evidence recorded separately from vanity metrics",
            "actual_result recorded",
            "decision recorded as SCALE, ITERATE, HOLD, or KILL",
            "no substantial build proceeds without active revenue evidence or an explicit infrastructure/research-only exception",
        ),
        approval_sensitive=True,
    ),
    PipelinePhase(
        ...
        (
            ...
            "scope limited to what materially improves transaction/lead capture, trust, measurement, fulfillment, or chosen-channel discovery before first revenue",
        ),
        approval_sensitive=True,
    ),
    PipelinePhase(
    ...
    PipelinePhase(
        ...
        (
            ...
            "next revenue test remains active until first paid customer or explicit HOLD/KILL decision",
        ),
        approval_sensitive=True,
    ),
    PipelinePhase(
        ...
        (
            ...
            "SCALE, ITERATE, HOLD, or KILL decision updated from verified commercial evidence",
        ),
    ),
)
